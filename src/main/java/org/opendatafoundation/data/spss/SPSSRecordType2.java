...
public class SPSSRecordType2 extends SPSSAbstractRecordType {
  ...
  /* The value label set associated with this variableRecord */ SPSSRecordType3 valueLabelSet;
  ...
  public void read(SPSSFile is) throws IOException, SPSSFileException {
    ...
    if(recordTypeCode != 2) throw new SPSSFileException(
        "Error reading variable Record: bad record type [" + recordTypeCode + "]. Expecting Record Type 2.");
    // variableRecord type
    ...
    if(Math.abs(missingValueFormatCode) > 3) throw new SPSSFileException(
        "Error reading variable Record: invalid missing value format code [" + missingValueFormatCode +
            "]. Range is -3 to 3.");
  ...
  public String toString() {
    ...
    str += "\n- Type          : " + printFormatType + " [" + getFormatTypeCode(printFormatType) + "/" +
        getFormatTypeLabel(printFormatType) + "]";
    ;
    str += "\n- Zero          : " + printFormatZero;
    str += "\nWrite Format    : " + writeFormatCode;
    str += "\n- Decimals      : " + writeFormatDecimals;
    str += "\n- Width         : " + writeFormatWidth;
    str += "\n- Type          : " + writeFormatType + " [" + getFormatTypeCode(writeFormatType) + "/" +
        getFormatTypeLabel(writeFormatType) + "]";
    ;
    ...
    if(missingValueFormatCode > 0) {
      ...
      for(int i = 0; i < missingValueFormatCode; i++) {
        str += "\nMissing " + i + "       :";
        if(variableTypeCode == 0) str += "" + SPSSUtils.byte8ToDouble(missingValue[i]);
        else str += (file.charset == null
            ? SPSSUtils.byte8ToString(missingValue[i])
            : SPSSUtils.byte8ToString(missingValue[i], file.charset));
      }
    } else if(missingValueFormatCode <= -2) {
      // -2 --> range of missing value codes
      str += "\nMissing range   : ";
      if(variableTypeCode == 0) str += "" + SPSSUtils.byte8ToDouble(missingValue[0]);
      else str += (file.charset == null
          ? SPSSUtils.byte8ToString(missingValue[0])
          : SPSSUtils.byte8ToString(missingValue[0], file.charset));
      str += " - ";
      if(variableTypeCode == 0) str += "" + SPSSUtils.byte8ToDouble(missingValue[1]);
      else str += (file.charset == null
          ? SPSSUtils.byte8ToString(missingValue[1])
          : SPSSUtils.byte8ToString(missingValue[1], file.charset));
      if(missingValueFormatCode == -3) {
        str += "\nMissing 3       : ";
        if(variableTypeCode == 0) str += "" + SPSSUtils.byte8ToDouble(missingValue[2]);
        else str += (file.charset == null
            ? SPSSUtils.byte8ToString(missingValue[2])
            : SPSSUtils.byte8ToString(missingValue[2], file.charset));
      }
    }
    return (str);
  }
